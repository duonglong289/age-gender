...
def load_my_state_dict(arch, model, state_dict=None, progress=False):
...
def _resnet(
    ...
    **kwargs: Any
    ...
    model = ResNet(block, layers, 
        num_age_classes=kwargs.get('num_age_classes', 81),
        num_gender_classes=kwargs.get('num_gender_classes', 2)
    )
    if pretrained:
        progress = True
        model = load_my_state_dict(arch, model, progress=progress)
    return model
